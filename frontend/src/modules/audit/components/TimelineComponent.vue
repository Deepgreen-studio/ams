<template>
  <ol class="relative space-y-4 border-l border-slate-200 pl-6">
    <li v-for="item in items" :key="item.id || item.uuid" class="relative">
      ...
      <p class="text-sm font-medium text-slate-900">{{ item.description || item.event || item.message }}</p>
      <p class="text-xs text-slate-500">{{ item.module || item.exception || item.method }} · {{ formatDate(item.created_at || item.login_at) }}</p>
    </li>
