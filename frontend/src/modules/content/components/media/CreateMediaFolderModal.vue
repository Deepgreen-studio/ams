<template>
  ...
  >
    ...
    >
      <h3 id="create-folder-title" class="text-lg font-semibold text-slate-900">New folder</h3>
      <p class="mt-1 text-sm text-slate-600">
        {{
          parentLabel
            ? `Create a folder inside “${parentLabel}”.`
            : 'Create a folder in the media library root.'
        }}
      </p>
      ...
      <form class="mt-5 space-y-4" @submit.prevent="onSubmit">
        <div>
          ...
          <input
            ...
            required
            autocomplete="off"
            class="w-full h-12 rounded-[12px] border border-slate-300 px-3 text-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-200"
