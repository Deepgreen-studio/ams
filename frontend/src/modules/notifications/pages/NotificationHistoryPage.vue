    ...
    <div v-else class="overflow-hidden rounded-[12px] bg-white ring-1 ring-zinc-100">
      <table class="min-w-full divide-y divide-zinc-100 text-sm">
        <thead class="bg-zinc-50 text-left text-xs uppercase tracking-wide text-slate-500">
          <tr>
            <th class="px-5 py-3.5">Notification</th>
        ...
        <tbody class="divide-y divide-zinc-100">
          <tr v-if="!store.items.length">
            <td colspan="5" class="px-5 py-12 text-center text-slate-500">No notifications found.</td>
          </tr>
          <tr v-for="item in store.items" :key="item.uuid" class="hover:bg-zinc-50/80">
            <td class="px-5 py-4">
            ...
            <td class="px-5 py-4 text-slate-600">{{ item.channel_label || item.channel }}</td>
            <td class="px-5 py-4 text-slate-600">{{ item.priority_label || item.priority }}</td>
            <td class="px-5 py-4">
              <button
